...
from django.contrib.auth.models import User
from .models import Customer  # Import your Customer model if needed

def Login_User(request):
    error = ""
    if request.method == "POST":
        u = request.POST.get('uname')  # Using `get` for safety
        ...
        if user:  # Check if authentication was successful
            if user.is_staff:  # Check if the user is a staff member
                login(request, user)
                error = "admin"
            else:  # Non-staff users
                try:
                    sign = Customer.objects.get(user=user)  # Verify if user is a customer
                    login(request, user)
                    error = "pat1"
                except Customer.DoesNotExist:  # Handle case where Customer object doesn't exist
                    error = "not"
...
def All_Movies(request,pid):
    ...
    if pid == 2:
        ...
        data = Movie.objects.filter(language=lang)
    if pid == 3:
        language = "Malayalam Movies"
        lang = Language.objects.get(name="Malayalam")
        data = Movie.objects.filter(language=lang)
    if pid == 4:
        language = "Malayalam Movies"
        lang = Language.objects.get(name="Tamil")
        data = Movie.objects.filter(language=lang)
...
def delete_booking(request, pid):
    ...
    # Use get_object_or_404 to handle the case when Booking doesn't exist
